...
exports.applyToJobSeekers = async (data) => {
    ...
    try {
        ...
        const pendingSql = data.program_id
            ? "SELECT application_id FROM applications WHERE user_id = ? AND program_type = 'job_seekers' AND program_id = ? AND status IN ('Pending', 'Approved')"
        ...
        const [appResult] = await connection.query(
            "INSERT INTO applications (user_id, program_type, program_id, status, applied_at) VALUES (?, 'job_seekers', ?, 'Pending', NOW())",

...
exports.applyToDilp = async (data) => {
    ...
    try {
        ...
        const pendingSql = data.program_id
            ? "SELECT application_id FROM applications WHERE user_id = ? AND program_type = 'dilp' AND program_id = ? AND status IN ('Pending', 'Approved')"
        ...
        const [appResult] = await connection.query(
            "INSERT INTO applications (user_id, program_type, program_id, status, applied_at) VALUES (?, 'dilp', ?, 'Pending', NOW())",
